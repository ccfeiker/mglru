...
static __always_inline void account_isolate(struct active_session *sample, int lru, unsigned long long nr_taken)
{
    ...
    case LRU_LIST_INACTIVE_ANON:
        ...
        sample->observed_shrink_input_anon_pages += nr_taken;
        break;
    ...
    case LRU_LIST_INACTIVE_FILE:
        ...
        sample->observed_shrink_input_file_pages += nr_taken;
        break;
...
static __always_inline unsigned long long nonnegative_delta(unsigned long long total, unsigned long long used)
{
    if (total > used)
        return total - used;
    return 0;
...
int BPF_KRETPROBE(shrink_lruvec_exit)
{
    ...
    event->observed_shrink_input_anon_pages = sample->observed_shrink_input_anon_pages;
    event->observed_shrink_input_file_pages = sample->observed_shrink_input_file_pages;
    ...
    event->observed_dirty_anon_pages = sample->observed_dirty_anon_pages;
    event->observed_dirty_file_pages = sample->observed_dirty_file_pages;
    event->observed_writeback_anon_pages = sample->observed_writeback_anon_pages;
    event->observed_writeback_file_pages = sample->observed_writeback_file_pages;
    event->observed_congested_anon_pages = sample->observed_congested_anon_pages;
    event->observed_congested_file_pages = sample->observed_congested_file_pages;
    event->observed_immediate_anon_pages = sample->observed_immediate_anon_pages;
    ...
    event->observed_putback_anon_pages = nonnegative_delta(
        sample->observed_shrink_input_anon_pages,
        sample->observed_reclaimed_anon_pages +
            sample->observed_activated_anon_pages +
            sample->observed_ref_keep_anon_pages +
            sample->observed_dirty_anon_pages +
            sample->observed_writeback_anon_pages +
            sample->observed_congested_anon_pages +
            sample->observed_immediate_anon_pages +
            sample->observed_unmap_fail_anon_pages);
    event->observed_putback_file_pages = nonnegative_delta(
        sample->observed_shrink_input_file_pages,
        sample->observed_reclaimed_file_pages +
            sample->observed_activated_file_pages +
            sample->observed_ref_keep_file_pages +
            sample->observed_dirty_file_pages +
            sample->observed_writeback_file_pages +
            sample->observed_congested_file_pages +
            sample->observed_immediate_file_pages +
            sample->observed_unmap_fail_file_pages);
...
int lru_shrink_inactive_tp(struct trace_event_raw_mm_vmscan_lru_shrink_inactive *ctx)
{
    ...
    if (file) {
        ...
        sample->observed_dirty_anon_pages += nr_dirty;
        sample->observed_writeback_anon_pages += nr_writeback;
        sample->observed_congested_anon_pages += ctx->nr_congested;
        sample->observed_immediate_anon_pages += ctx->nr_immediate;
