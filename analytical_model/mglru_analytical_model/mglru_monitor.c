...
static void write_csv_header(FILE *fp)
{
    ...
    fprintf(fp, ",feature_nr_scanned_before,feature_nr_scanned_after,feature_nr_scanned_pages");
    fprintf(fp, ",feature_nr_reclaimed_before,feature_nr_reclaimed_after");
    fprintf(fp, ",feature_evict_folios_calls");
    fprintf(fp, ",feature_evict_folios_return_sum");
    fprintf(fp, ",feature_evict_folios_calls_anon,feature_evict_folios_calls_file");
    fprintf(fp, ",feature_evict_folios_return_sum_anon,feature_evict_folios_return_sum_file");
    fprintf(fp, ",feature_evict_scanned_delta_anon_pages,feature_evict_scanned_delta_file_pages");
    fprintf(fp, ",feature_isolate_calls_anon,feature_isolate_calls_file");
    fprintf(fp, ",feature_isolate_scanned_anon_pages,feature_isolate_scanned_file_pages");
    ...
    fprintf(fp, ",label_reclaimed_anon_pages,label_reclaimed_file_pages");
    fputc('\n', fp);
}
...
static void write_csv_event(FILE *fp, const struct mglru_event *event)
{
    ...
    fprintf(fp, ",%d,%d,%d,%d,%u,%u,%llu,%llu",
            event->swappiness,
            event->priority,
            event->reclaim_idx,
            event->order,
            ...
            event->gfp_mask,
            event->nr_to_reclaim);
    fprintf(fp, ",%llu,%llu,%llu,%llu,%llu,%u,%llu",
            event->nr_scanned_before,
            event->nr_scanned_after,
            event->nr_scanned_delta,
            event->nr_reclaimed_before,
            event->nr_reclaimed_after,
            event->evict_folios_calls,
            event->evict_folios_return_sum);
    fprintf(fp, ",%u,%u,%llu,%llu,%llu,%llu",
            event->evict_folios_calls_by_type[0],
            event->evict_folios_calls_by_type[1],
            event->evict_folios_return_sum_by_type[0],
            event->evict_folios_return_sum_by_type[1],
            event->evict_scanned_delta_pages[0],
            event->evict_scanned_delta_pages[1]);
    fprintf(fp, ",%u,%u,%llu,%llu",
            event->isolate_calls[0],
            event->isolate_calls[1],
            event->isolate_scanned_pages[0],
            event->isolate_scanned_pages[1]);
    fprintf(fp, ",%llu,%llu",
    ...
    fprintf(fp, ",%llu,%llu",
            event->label_reclaimed_anon_pages,
            event->label_reclaimed_file_pages);
    fputc('\n', fp);
    fflush(fp);
}
...
{
    printf("memcg_id=%llu trigger_comm=%s anon_min_gen_seq=%llu file_min_gen_seq=%llu max_gen_seq=%llu after_anon_min_gen_seq=%llu after_file_min_gen_seq=%llu after_max_gen_seq=%llu swappiness=%d priority=%d reclaim_idx=%d may_writepage=%u may_unmap=%u nr_scanned_delta=%llu evict_folios_calls=%u evict_folios_return_sum=%llu evict_calls_anon=%u evict_calls_file=%u evict_scanned_anon_pages=%llu evict_scanned_file_pages=%llu isolate_calls_anon=%u isolate_calls_file=%u isolate_scanned_anon_pages=%llu isolate_scanned_file_pages=%llu label_reclaimed_anon_pages=%llu label_reclaimed_file_pages=%llu\n",
           event->memcg_id,
           event->trigger_comm,
           event->before.min_seq[0],
           event->before.min_seq[1],
