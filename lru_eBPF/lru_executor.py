...
def reclaimable_cap_anon(case: CaseInput, config: ExecutorConfig, inactive_floor: int = 0) -> Tuple[int, Dict[str, int]]:
    scanned = case.observed_scanned_inactive_anon_pages or inactive_floor
    penalties = {
    ...
    cap = max(scanned - penalty_total, 0)
...
def reclaimable_cap_file(case: CaseInput, config: ExecutorConfig, inactive_floor: int = 0) -> Tuple[int, Dict[str, int]]:
    scanned = case.observed_scanned_inactive_file_pages or inactive_floor
    penalties = {
...
def reclaim_budget(case: CaseInput, type_name: str, targets: Dict[str, Any]) -> Dict[str, int]:
    ...
    if type_name == "anon":
        ...
        inactive_budget = case.observed_scanned_inactive_anon_pages or inactive_target
        active_budget = case.observed_taken_active_anon_pages or active_target
        ...
        inactive_budget = case.observed_scanned_inactive_file_pages or inactive_target
        active_budget = case.observed_taken_active_file_pages or active_target
        has_active_signal = any(
            (
    ...
    total_budget = inactive_budget + demoted_budget
